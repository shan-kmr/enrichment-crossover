#!/usr/bin/env python3
"""Train LightGBM and MLP on dual embeddings for Gowalla friendship prediction."""
...
def main():
    ...
    print("\n" + "=" * 90)
    print("GOWALLA FRIENDSHIP PREDICTION: EMBED-v2 (LightGBM + MLP)")
    print("=" * 90)
    ...
    for metric in ["accuracy", "f1"]:
        ...
        row_lgb = f"{'LightGBM (embed-v2)':<30}"
        row_mlp = f"{'MLP (embed-v2)':<30}"
        for level in levels:
            row_lgb += f"{all_results[level]['lgb_embed'][metric]:>10.4f}"
            row_mlp += f"{all_results[level]['mlp_embed'][metric]:>10.4f}"
        print(row_lgb)
        print(row_mlp)
    ...
    out_path = results_dir / "embed_results_v2.json"
