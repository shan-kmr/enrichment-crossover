#!/usr/bin/env python3
"""Prepare NON-ENRICHED JSONL files for Llama 3.1 8B ablation experiment.

Same filtered user pool (80% Overture coverage) as the enriched experiment,
but prompts use raw location IDs instead of venue category names.
This isolates the value of reverse geocoding.

Generates 5 train + 5 test JSONL files (no few-shot needed for ablation):
...
from src.gowalla.prompt_builder import build_prompt, SYSTEM
...
def main():
    ...
    out_dir = ROOT / "data" / "processed" / "gowalla" / "llama_jsonl_nonenriched"
    ...
    print(f"\nAll non-enriched JSONL files saved to {out_dir}/")
