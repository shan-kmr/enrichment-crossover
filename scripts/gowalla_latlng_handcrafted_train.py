#!/usr/bin/env python3
"""Train LightGBM and MLP on pure lat/lng handcrafted features (no venue IDs, no categories)."""
...
from src.gowalla.latlng_features import compute_latlng_handcrafted
...
def main():
    ...
    for level in levels:
        print(f"\n{'='*60}")
        print(f"Level: {level} (pure lat/lng handcrafted)")
        print(f"{'='*60}")

        X_train = np.array(
            [compute_latlng_handcrafted(tc, level) for tc in train_cases],
            dtype=np.float32,
        )
        X_test = np.array(
            [compute_latlng_handcrafted(tc, level) for tc in test_cases],
            dtype=np.float32,
        )
        ...
        all_results[level] = {"lgb": m_lgb, "mlp": m_mlp}

    out_path = results_dir / "latlng_handcrafted_results.json"
