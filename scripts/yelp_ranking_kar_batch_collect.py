#!/usr/bin/env python3
"""Collect KAR batch results for ranking task, embed reasoning, save .npy.

Usage:
  python3 scripts/yelp_ranking_kar_batch_collect.py --status   # check progress
  python3 scripts/yelp_ranking_kar_batch_collect.py --wait      # poll then process
  python3 scripts/yelp_ranking_kar_batch_collect.py             # process if done
"""
...
def main():
    ...
    info = json.loads((batch_dir / "ranking_batch_info.json").read_text())
    ...
    for split, cases in [("train", train_cases), ("test", test_cases)]:
        ...
        if not hc_path.exists():
            ...
            hc = np.array(
                [[compute_handcrafted(tc, c) for c in tc.candidates] for tc in cases],
                dtype=np.float32,
            )
            np.save(hc_path, hc)
        ...
        for level in levels:
            ...
            for ci in range(len(cases)):
                for cj in range(n_cands):
                    key = f"{split}|{level}|{ci:04d}|{cj:02d}"
            ...
            if missing:
                print(f"  WARNING: {missing} missing for {split}/{level}")

            print(f"  Embedding {split}/{level}: {len(reasoning_texts)} texts...")
            embs_flat = embed_texts(embed_client, reasoning_texts, embed_cache)
    ...
    print(f"Next: python3 scripts/yelp_ranking_kar_train.py")
