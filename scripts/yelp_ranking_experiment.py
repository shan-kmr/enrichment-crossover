#!/usr/bin/env python3
"""Run the Yelp next-business candidate ranking experiment."""
...
from src.yelp.ranking_prompt_builder import build_ranking_prompt
...
def run_condition(client, model, granularity, test_cases, cfg, results_dir):
    ...
    for i, tc in enumerate(tqdm(test_cases, desc=f"  {model} / {granularity}", leave=False)):
        ...
        messages = build_ranking_prompt(tc, granularity)
        ...
        try:
            ...
            results.append({
                "index": i, "user_id": tc.user_id, "error": str(e),
                "ranking": None, "ground_truth_idx": tc.ground_truth_idx,
            })
        ...
        results.append({
            ...
            "ranking": ranking, "ground_truth_idx": tc.ground_truth_idx,
...
def main():
    ...
    if not test_cases_path.exists():
        print("ERROR: Run scripts/yelp_ranking_preprocess.py first.")
        sys.exit(1)

    print("Loading test cases...")
    test_cases = load_ranking_test_cases(test_cases_path)
    ...
    print(f"\nRunning {len(models) * len(levels)} conditions")
