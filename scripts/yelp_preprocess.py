#!/usr/bin/env python3
"""Preprocess Yelp Open Dataset into test cases for rating prediction."""
...
from src.yelp.data_loader import build_test_cases, load_yelp_data, save_test_cases
...
def main():
    ...
    for p, label in [(biz_path, "business"), (rev_path, "review")]:
        if not p.exists():
            ...
            print(f"Place yelp_academic_dataset_{label}.json in {p.parent}/")
            sys.exit(1)
    ...
    print(f"Building test cases (min {ds['min_user_reviews']} reviews/user, sampling {ds['test_sample_size']})...")
    test_cases = build_test_cases(
        df,
        min_user_reviews=ds["min_user_reviews"],
        test_reviews_per_user=ds["test_reviews_per_user"],
